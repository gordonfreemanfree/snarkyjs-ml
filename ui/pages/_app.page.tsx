...
export default function App(props: any) {
  ...
  // const [weightsLayer1, setWeightsLayer1] = useState<Array<Field>[]>(Array<Field>[])
  // const [weightsLayer2, setWeightsLayer2] = useState<Array<Field>[]>(Array<Field>[])
  ...
  function handleSavedImageData(data: number[]) {
    setSavedImageData(data)
  }

  function handleScaledImage(data: number[]) {
    setScaledImage(data)
    // setSelectedImage(data!.map((x) => Field(x)))
  }
  ...
  useEffect(() => {
    ;(async () => {
      if (!state.hasBeenSetup) {
        ...
        toast.update(toastLoading, { render: "SnarkyJS loaded", type: "success", isLoading: false, autoClose: 2000 });
        console.log('done')
        ...
        toast.update(toastCompiling, { render: "zkApp compiled", type: "success", isLoading: false, autoClose: 2000 });
        ...
        toast.update(toastZkapppublickey, { render: "Using zkApp public key B62qrSgTkaLZAJjcgBp7SH7BXoN4UdgxBwRiBx9mJ34TyivW2MAV3KP", type: "success", isLoading: false, autoClose: 2000 });
        
        // Defining the weights for both layers
  ...
  // call predict function and if it succeeds, update the state
  const onSendPredictTransaction = async () => {
    setState({ ...state, creatingTransaction: true })
    console.log('sending a predict transaction...')

    notify('Fetching account state...')
    ...
    const toastTransaction = toast.loading('Creating transaction...')
    await state.zkappWorkerClient!.createPredictUpdateTransaction({ selectedImage, weights_l1_8x8, weights_l2_8x8 })
    toast.update(toastTransaction, { render: "Transaction created...", type: "success", isLoading: false, autoClose: 2000 });

    console.log('creating proof...')
    const toastProof = toast.loading('Creating proof...')
    await state.zkappWorkerClient!.proveUpdateTransaction()
    toast.update(toastProof, { render: "Proof created...", type: "success", isLoading: false, autoClose: 2000 });
    console.log('getting Transaction JSON...')

    notify('Getting transaction JSON...')
    const transactionJSON = await state.zkappWorkerClient!.getTransactionJSON()

    console.log('requesting send transaction...')
    notify('Requesting send transaction...')
    ...
    notify( 'See transaction at https://berkeley.minaexplorer.com/transaction/' + hash)
  ...
  if (state.hasBeenSetup && state.accountExists) {
    mainContent = (
      <div>
      <Demo setSelectedImage={setSelectedImage} selectedImage={selectedImage} handleSavedImageData={handleSavedImageData} handleScaledImage={handleScaledImage} onSendPredictTransaction={onSendPredictTransaction} creatingTransaction={state.creatingTransaction} onRefreshCurrentNum={ onRefreshCurrentNum} state={state} />
      <Description />
      <Usecases />
      <Acknowledgement />
        {/* <div> Current Number in zkApp: {state.currentNum!.toString()} </div>
        <button onClick={onRefreshCurrentNum}> Get Latest State </button>
        <button onClick={onSendPredictTransaction} disabled={state.creatingTransaction}> Send predict button huge </button> */}
